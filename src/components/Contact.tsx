  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          access_key: 'aba5b5b7-0c39-4ee9-857e-b0af7c338972',
          name: name,
          email: email,
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            {/* Quick Email Card */}
            <div className="p-6 bg-slate-900 border border-slate-800 rounded-2xl flex items-start space-x-4">
              ...
              <div className="flex-1">
                ...
                <a 
                  ...
                  className="font-display font-medium text-[15px] text-white hover:text-sky-400 transition-colors block mt-1"
                >
            ...
            <div className="p-6 bg-slate-900 border border-slate-800 rounded-2xl">
              ...
              <div className="space-y-3.5">
                <a
                  ...
                  className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/50 border border-slate-800 hover:bg-slate-800/40 text-slate-300 hover:text-white hover:border-slate-700 transition duration-250 cursor-pointer"
                >
                ...
                <a
                  href="https://www.linkedin.com/in/yenersonolivo/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/50 border border-slate-800 hover:bg-slate-800/40 text-slate-300 hover:text-white hover:border-slate-700 transition duration-250 cursor-pointer"
                >
                  <div className="flex items-center space-x-2.5">
                    ...
                    <span className="text-xs font-semibold">LinkedIn (Yennerson Olivo)</span>
                  </div>
                  <ExternalLink className="w-3.5 h-3.5 text-slate-500" />
                </a>
              </div>
          ...
          <div className="lg:col-span-8 bg-slate-900 border border-slate-800 rounded-3xl p-8 relative">
            {isSuccess && (
              <div className="absolute inset-0 bg-slate-900/98 rounded-3xl flex flex-col justify-center items-center p-8 text-center animate-in fade-in duration-300 z-20">
                ...
                <p className="text-slate-400 max-w-sm mt-3 font-sans">
                  Gracias por comunicarte conmigo. He recibido tu mensaje e iniciaré el contacto inmediato.
                </p>
                <button
                  onClick={() => setIsSuccess(false)}
                  className="mt-6 px-5 py-2 rounded-xl bg-slate-800 text-slate-300 border border-slate-700 text-xs font-semibold hover:bg-slate-700 hover:text-white transition"
                >
                  Seguir Navegando
                </button>
              </div>
            ...
            <form onSubmit={handleSendMessage} className="space-y-5">
              <h3 className="font-display font-bold text-white text-lg border-b border-slate-800 pb-3 block">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-sky-500 font-sans"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full flex items-center justify-center space-x-2 py-3.5 rounded-xl text-sm font-bold bg-gradient-to-r from-sky-500 to-indigo-500 text-white hover:opacity-95 transform active:translate-y-0 hover:-translate-y-0.5 transition duration-200 cursor-pointer disabled:opacity-70"
              >
                {isLoading ? (
